...
#                 "/../../MotionPlanning/")
sys.path.append(os.path.join(os.path.expanduser("~/robinz_ws/src/robinz_path_planner/"), "CurvesGenerator/"))

# Now you can import from CurvesGenerator
...
def extract_optimal_path(paths):
    ...
    # print(f"Paths available: {paths}")  # Debugging output

    while len(paths) > 1:
        # Find the path with the minimum cost (or other criteria)
        path = min(paths, key=paths.get)
        paths.pop(path)  # Remove the selected path from the dictionary
        
        # Verify the path
        if verify_path(path) is False:
            continue  # If the path is not valid, continue the loop

        return path  # Return the valid optimal path

    # If there's only one path left, return it
    if paths:  # Ensure paths is not empty
        return paths.popitem()[0]  # Pop and return the last remaining item

    print("No valid paths left.")  # Debugging output
    return None  # Return None if there are no valid paths left
